...
estimate_mortality(crab, "arm", "sw")

crab_bsi$arm <- droplevels(crab_bsi$arm)
estimate_mortality(crab_bsi, "arm", "w2")

crab_vap$arm <- droplevels(crab_vap$arm)
estimate_mortality(crab_vap, "arm", "w2")
...
cre_crpae_mono$arm3 <- droplevels(cre_crpae_mono$arm3)
table(cre_crpae_mono$arm3)
estimate_mortality(cre_crpae_mono, "arm3", "w2")


cre_crpae_bsi$arm3 <- droplevels(cre_crpae_bsi$arm3)
table(cre_crpae_bsi$arm3)
estimate_mortality(cre_crpae_bsi, "arm3", "w2")

cre_crpae_vap$arm3 <- droplevels(cre_crpae_vap$arm3)
table(cre_crpae_vap$arm3)
estimate_mortality(cre_crpae_vap, "arm3", "w2")
...
# 
# # Print class of all columns in crab
# sapply(crab, class)
# # Print class of all columns in cre_crpae
# sapply(cre_crpae, class)
...
# sensitivity 1 (delay group, selected comorb)
multinom_model <- multinom(as.formula(paste(treatment_var, "~ age_new  + sex + country_income2 + 
...
# sensitivity 2 (quadratic)
multinom_model <- multinom(as.formula(paste(treatment_var, "~ age_new + I(age_new*age_new) + sex + country_income2 + 
                                                comorbidities_Chalson + I(comorbidities_Chalson*comorbidities_Chalson) +
                                              sofa_imp + I(sofa_imp*sofa_imp) + infection_types + 
                                              los_onset4 + I(los_onset4*los_onset4) + hai_icu48days + hai_have_med_device___vent + monopoly +
                                              delay + I(delay*delay) + ent + pae")), 
                           data = data)
...
# sensitivity 4 (day 5) in separate code
...
estimate_mortality <- function(data, treatment_var, weight_var) {
  ...
  # ---- Crude model ----
  crude_model <- glm(mort_14d_onset4 ~ treatment, data = data, family = "binomial")
  crude_newdata <- data.frame(treatment = levels(factor(data$treatment)))
  pred_crude <- predict(crude_model, newdata = crude_newdata, type = "link", se.fit = TRUE)
  crude_newdata$risk <- plogis(pred_crude$fit)
  crude_newdata$lower <- plogis(pred_crude$fit - 1.96 * pred_crude$se.fit)
  ...
  # ---- MSM model using final weights ----
  msm_model <- glm(mort_14d_onset4 ~ treatment, data = data, family = "binomial", weights = final_weight)
  adjusted_newdata <- data.frame(treatment = levels(factor(data$treatment)))
  pred_adj <- predict(msm_model, newdata = adjusted_newdata, type = "link", se.fit = TRUE)
  adjusted_newdata$risk <- plogis(pred_adj$fit)
  adjusted_newdata$lower <- plogis(pred_adj$fit - 1.96 * pred_adj$se.fit)
  ...
  plot <- ggplot(result_table, aes(x = risk, y = treatment)) +
    ...
    geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0.2) +
    facet_wrap(~ analysis, ncol = 1, scales = "free_y") +
    scale_x_continuous(name = "Estimated 14-Day Mortality Risk (%)", limits = c(0, 100)) +
    labs(y = "Treatment", title = paste("14-Day Mortality Risk by", treatment_var, "with", weight_var)) +
    theme_minimal(base_size = 14)
